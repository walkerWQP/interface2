...
- (void)dismiss {
    [self dismissAnimated:_showAnimated completion:nil];
}
        ...
        if (photoImg) {
            [photoSheet bk_addButtonWithTitle:@"保存图片" handler:^{
                [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
                    //写入图片到相册
                    [PHAssetChangeRequest creationRequestForAssetFromImage:photoImg];
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        // UI更新代码
                        if (success) {
                            [WProgressHUD showSuccessfulAnimatedText:@"保存成功!"];
                        } else {
                            [WProgressHUD showErrorAnimatedText:@"保存失败, 请打开相册权限"];
                        }
                    });
